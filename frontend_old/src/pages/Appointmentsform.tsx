...
const Appointments = () => {
  ...
  const handleMedicalHistoryChange = (checked: boolean | "indeterminate") => {
    if (checked === true) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await api.post<{ message?: string }>("/appointments", submitData);
      ...
      toast.error(typeof detail === "string" ? detail : "Failed to book appointment");
    }
  };
              ...
              <div className="flex items-center space-x-3">
                <Checkbox 
                  ...
                  onCheckedChange={(checked) =>
                    setFormData({ ...formData, vaccination_status: checked === true })
                  }
                />
